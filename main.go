...
func GetNumbersWhichSums(digits []int, total int) (result [][]int) {
	...
	log.Printf("Detected %v results, %v of %v possible combinations tested", len(result), testedCases, math.Pow(2, float64(len(digits)))-1)
...
func main() {
	...
	r := GetNumbersWhichSums([]int{-1, 1, 1, 2, 2}, 3)
	fmt.Printf("%v\n", r)
}
